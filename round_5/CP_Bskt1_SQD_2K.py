...
class Trader:
    def run(self, state: TradingState):
        ...
        squid_orders = self.generate_squid_ink_orders(state, trader_data["SQUID_INK"])
        if squid_orders:
    ...
    def update_trade_data(self, state, trader_data):
        ...
        if "SQUID_INK" in state.own_trades and state.own_trades["SQUID_INK"]:
            for trade in state.own_trades["SQUID_INK"]:
                ...
                if len(trader_data["SQUID_INK"]["last_trades"]) > 10:
                    trader_data["SQUID_INK"]["last_trades"] = trader_data["SQUID_INK"]["last_trades"][-10:]
    ...
    def generate_squid_ink_orders(self, state, product_data):
        """Generate orders for PICNIC_BASKET1 focusing on Charlie"""
        ...
        # Charlie-specific thresholds based on analysis
        # Charlie buys at 58412.94 and sells at 58350.33 - extremely profitable!
        caesar_buy_threshold = 2040 # Price at which Charlie tends to buy
        gary_sell_threshold = 1945 # Price at which Charlie tends to sell
        
        # Calculate average Charlie prices if we have data
        if product_data["gary_buy_prices"]:
            recent_prices = product_data["gary_buy_prices"][-10:] if len(product_data["gary_buy_prices"]) > 10 else product_data["gary_buy_prices"]
            avg_charlie_buy = sum(recent_prices) / len(recent_prices)
            caesar_buy_threshold = avg_charlie_buy
        
        if product_data["caesar_sell_prices"]:
            recent_prices = product_data["caesar_sell_prices"][-10:] if len(product_data["caesar_sell_prices"]) > 10 else product_data["caesar_sell_prices"]
            avg_gary_sell = sum(recent_prices) / len(recent_prices)
            gary_sell_threshold = avg_gary_sell
        ...
        if max_long_capacity > 0 and not (avoid_new_positions and current_position >= 0):
            ...
            for price in sell_prices:
                ...
                # Standard buy logic - look for prices below Charlie's selling threshold
                if price <= gary_sell_threshold * 1.0001:
                    volume = abs(order_depth.sell_orders[price])
                    
                    # Determine buy quantity based on how good the price is
                    if price < gary_sell_threshold * 0.998:  # Very good price
                        buy_quantity = min(volume, max_long_capacity, 12)
                    elif price < gary_sell_threshold * 0.999:  # Good price
                        buy_quantity = min(volume, max_long_capacity, 8)
                    else:  # Acceptable price
                        buy_quantity = min(volume, max_long_capacity, 5)
        ...
        if max_short_capacity > 0 and not (avoid_new_positions and current_position <= 0):
            ...
            for price in buy_prices:
                ...
                # Standard sell logic - look for prices above Charlie's buying threshold
                if price >= caesar_buy_threshold * 0.9999:
                    volume = order_depth.buy_orders[price]
                    
                    # Determine sell quantity based on how good the price is
                    if price > caesar_buy_threshold * 1.002:  # Very good price
                        sell_quantity = min(volume, max_short_capacity, 12)
                    elif price > caesar_buy_threshold * 1.001:  # Good price
                        sell_quantity = min(volume, max_short_capacity, 8)
                    else:  # Acceptable price
                        sell_quantity = min(volume, max_short_capacity, 5)
    ...
    # def generate_squid_ink_orders(self, state, product_data):
    #     """Generate simple orders for SQUID_INK to ensure we're trading - simpler approach"""
    #     target_product = "SQUID_INK"
    ...
    #     # Simple approach: Use default gary thresholds as starting point
    #     gary_buy_threshold = 1960.0  # gary buys high
    #     gary_sell_threshold = 1810.0  # gary sells low
        
    #     # If we have counterparty data, use it
    #     if product_data["gary_buy_prices"]:
    #         recent_prices = product_data["gary_buy_prices"][-5:] if len(product_data["gary_buy_prices"]) > 5 else product_data["gary_buy_prices"]
    #         avg_gary_buy = sum(recent_prices) / len(recent_prices)
    #         gary_buy_threshold = avg_gary_buy
        
    #     if product_data["gary_sell_prices"]:
    #         recent_prices = product_data["gary_sell_prices"][-5:] if len(product_data["gary_sell_prices"]) > 5 else product_data["gary_sell_prices"]
    #         avg_gary_sell = sum(recent_prices) / len(recent_prices)
    #         gary_sell_threshold = avg_gary_sell
        
    #     # If we don't have any gary data, use a fixed midpoint and margin
    #     if not product_data["gary_buy_prices"] and not product_data["gary_sell_prices"]:
    #         # Start with a more aggressive strategy to get initial trades
    #         # We'll assume any price better than mid +/- 1% is potentially good
    #         gary_buy_threshold = mid_price * 1.01
    #         gary_sell_threshold = mid_price * 0.99
        
    #     # Simple buy orders logic - buy when price is good
    #     if max_long_capacity > 0:
    #         sell_prices = sorted(order_depth.sell_orders.keys())
            
    #         for price in sell_prices:
    #             # Simple buy logic - looking for prices below threshold
    #             if price <= mid_price:  # Buy anything below mid price to start
    #                 volume = abs(order_depth.sell_orders[price])
                    
    #                 # Conservative quantity to start with
    #                 buy_quantity = min(volume, max_long_capacity, 5)
    ...
    #                     orders.append(Order(target_product, price, buy_quantity))
    #                     max_long_capacity -= buy_quantity
    #                     break  # Just do one trade to start
        
    #     # Simple sell orders logic - sell when price is good
    #     if max_short_capacity > 0:
    #         buy_prices = sorted(order_depth.buy_orders.keys(), reverse=True)
            
    #         for price in buy_prices:
    #             # Simple sell logic - looking for prices above threshold
    #             if price >= mid_price:  # Sell anything above mid price to start
    #                 volume = order_depth.buy_orders[price]
    ...
    #     # Simple risk management - if position is large, reduce it
    #     if current_position > 15 and order_depth.buy_orders:
    ...
    #     elif current_position < -15 and order_depth.sell_orders:
    ...
    def generate_risk_management_orders_squid(self, product, order_depth, product_data, current_position, stop_loss_triggered):
        """Generate orders for risk management (stop loss and profit taking) for SQUID_INK"""
        orders = []
        ...
        # If stop loss is triggered, reduce position - same as PICNIC_BASKET1
        if stop_loss_triggered:
            if current_position > 0 and order_depth.buy_orders:
                # Sell some position at market (best bid)
                best_price = max(order_depth.buy_orders.keys())
        ...
        # If position is profitable, consider reducing it - using 1.5% threshold as specified
        elif average_position_price > 0:
            # Get mid price
            if order_depth.buy_orders and order_depth.sell_orders:
                best_bid = max(order_depth.buy_orders.keys())
                ...
                # Profit thresholds - 1.5% as per your requirements
                profit_threshold = 0.015  # 1.5% 
                ...
                if current_position > 5 and mid_price > average_position_price * (1 + profit_threshold):
                    ...
                    if order_depth.buy_orders:
                        best_price = max(order_depth.buy_orders.keys())
                        volume = order_depth.buy_orders[best_price]
                        # Sell up to 40% of our position as per your requirements
                        sell_quantity = min(volume, int(current_position * 0.4))
                        if sell_quantity > 0:
                            orders.append(Order(product, best_price, -sell_quantity))
                    ...
                    if order_depth.sell_orders:
                        best_price = min(order_depth.sell_orders.keys())
                        volume = abs(order_depth.sell_orders[best_price])
                        # Buy up to 40% of our position as per your requirements
                        buy_quantity = min(volume, int(abs(current_position) * 0.4))
                        if buy_quantity > 0:
                            orders.append(Order(product, best_price, buy_quantity))
